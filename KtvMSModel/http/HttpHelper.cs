...
namespace KtvMSModel
{
    ...
    {
        private static bool RemoteCertificateValidate(object sender, X509Certificate certificate,X509Chain chain,SslPolicyErrors errose)
        {
            return true;///用户的http请求，总是接受
        }
        ...
        public static void DownloadFile(string url,string taget)
        {
            var config = new HttpConfig();
            try
            {
                using (HttpWebResponse response=GetResponse(url,"GET",null, config))
                {
                    ...
                    using (var fileStream=new FileStream(taget,FileMode.Create))
                    {
                        int count;
        ...
        public static string Send(string url,string method,string data,HttpConfig config)
        {
            ...
            using (HttpWebResponse response=GetResponse(url,method,data,config))
            {
                ...
                //检测响应头是否返回编码类型，若返回编码类型则使用编码类型
                //注：有响应头没有编码类型，CharacterSet经常设置为ISO-8859-1
        ...
        private static HttpWebResponse GetResponse(string url,string method,string data,HttpConfig config)
        {
            ...
            //设置用户代理，有些页面不设置用户代理信息则抓取不到内容
            request.UserAgent = config.UserAgent;
            request.Timeout = config.Timeout;
            request.Accept = config.Accept;
            request.Headers.Set("Accept-Encoding",config.AccpetEncoding);
    ...
    public class HttpConfig
    {
        ...
        public string AccpetEncoding { get; set; }
        /// <summary>
        /// 超时时间（毫秒）魔人100000
        /// </summary>
        ...
        {
            this.Timeout = 100000;
            this.ContentType = "text/html="+Encoding.UTF8.WebName;
            this.UserAgent = "Mozilla/5.0 (iPhon; CPU iPhone OS 9_1 like Mac OS X) AppleWebkit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1" ;
            this.Accept = "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8";
            this.AccpetEncoding = "gzip,deflate";
